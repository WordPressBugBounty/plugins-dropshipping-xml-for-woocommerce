...
class ListenerCollection implements ListenerCollectionInterface
{
    ...
    {
        if (is_string($class_name) && class_exists($class_name) && is_subclass_of($class_name, AbstractServiceListener::class)) {
            $this->listeners[$class_name] = new $class_name();
        } else {
            throw new RuntimeException('Listener class should implements ' . AbstractServiceListener::class);
        }
    }
}
