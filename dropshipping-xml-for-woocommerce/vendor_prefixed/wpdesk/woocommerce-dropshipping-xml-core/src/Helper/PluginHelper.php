...
class PluginHelper
{
    ...
    {
        $action = empty($action) ? MenuConfig::ACTION_DEFAULT : $action;
        ...
        if ($menus->isArray() && !$menus->isEmpty()) {
            foreach ($menus->get() as $menu) {
                if ($this->check_if_view_exists($menu, $page_id, $action)) {
                    return \true;
                } elseif ($menu instanceof MainMenuInterface) {
    ...
    {
        return $menu instanceof MenuInterface && $menu->get_slug() === $page_id && $menu->has_view_action($action);
    }
}
