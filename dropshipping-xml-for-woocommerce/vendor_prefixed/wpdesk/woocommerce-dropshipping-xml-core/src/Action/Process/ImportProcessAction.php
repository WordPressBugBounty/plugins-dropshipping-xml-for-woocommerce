 ...
 */
class ImportProcessAction implements Hookable
{
    ...
    public function process_ajax()
    {
        try {
            ...
            if (\in_array($file_import->get_status(), [Import::STATUS_ERROR, Import::STATUS_STOPPED])) {
                $this->reset_import_to_default($file_import);
                $this->import_dao->update($file_import);
            }
            Import::STATUS_WAITING === $file_import->get_status() ? $this->process_next_import($file_import) : $this->process_import($file_import);
    ...
    private function process_import(Import $file_import)
    {
        if (\true === $this->lock_import()) {
            switch ($file_import->get_status()) {
                case Import::STATUS_DOWNLOADING:
                    $this->process_downloading($file_import);
                    break;
                case Import::STATUS_CONVERTING:
                    $this->process_converting($file_import);
                    break;
                case Import::STATUS_IN_PROGRESS:
                    $this->process_import_progress($file_import);
                    break;
                case Import::STATUS_CLEANING:
    ...
    private function process_downloading(Import $file_import)
    {
        $this->logger->notice(__('Downloading', 'dropshipping-xml-for-woocommerce'));
        $data_provider = $this->data_provider_factory->create_by_class_name(ImportFileDataProvider::class, ['postfix' => $file_import->get_uid()]);
        ...
        $file_import->set_status(Import::STATUS_CONVERTING);
        $this->import_dao->update($file_import);
    }
    private function process_converting(Import $file_import)
    {
        $this->logger->notice(__('Converting', 'dropshipping-xml-for-woocommerce'));
        $import_data_provider = $this->data_provider_factory->create_by_class_name(ImportFileDataProvider::class, ['postfix' => $file_import->get_uid()]);
        $parameters = [];
        $data_format = $import_data_provider->get(ImportFileFormFields::ORIGINAL_FILE_FORMAT);
        $source_file = $this->file_locator->get_source_file($file_import->get_uid());
        if (DataFormat::CSV == $data_format) {
            $csv_data_provider = $this->data_provider_factory->create_by_class_name(ImportCsvSelectorDataProvider::class, ['postfix' => $file_import->get_uid()]);
            $parameters = ['separator' => $csv_data_provider->get(ImportCsvSelectorFormFields::INPUT_SEPARATOR), 'source_encoding' => $this->csv_analyser->resolve_source_encoding($source_file)];
        }
        ...
        $file_import->set_status(Import::STATUS_IN_PROGRESS);
        $this->import_dao->update($file_import);
    }
    private function process_import_progress(Import $file_import)
    {
        ...
        if ($this->import_service->is_finished()) {
            ...
            $file_save_import->set_status(Import::STATUS_CLEANING);
        }
        $this->import_dao->update($file_save_import);
    }
    private function process_cleaning(Import $file_import)
    {
    ...
    private function finish_process(Import $file_import)
    {
        $this->logger->notice(__('Finish', 'dropshipping-xml-for-woocommerce'));
        $this->reset_import_to_default($file_import);
        $this->finished = \true;
    }
    private function reset_import_to_default(Import $file_import): Import
    {
        ...
        $file_import->set_status(Import::STATUS_WAITING);
        return $file_import;
    }
    ...
    private function stop_import_with_error(Import $file_import, string $error_message = '')
    {
        $file_import->set_status(Import::STATUS_ERROR);
