...
class ImportXmlSelectorFormProcessAction implements Conditional, Initable
{
     ...
     */
    private $import_dao;
    public function __construct(Request $request, DataProviderFactory $data_provider_factory, ImportXmlSelectorForm $form, PluginHelper $helper, ImportDAO $import_dao)
    {
    ...
    private function save_form_data()
    {
        ...
        if ($this->form->is_valid() && current_user_can('manage_options')) {
            $data_provider = $this->data_provider_factory->create_by_class_name(ImportXmlSelectorDataProvider::class, ['postfix' => $uid]);
            ...
            if (!empty($mode)) {
                ...
                if ($this->import_dao->is_uid_exists($uid)) {
                    ...
                    $element = $data_provider->get(ImportXmlSelectorFormFields::NODE_ELEMENT);
            ...
            $url = $this->plugin_helper->generate_url_by_view(ImportMapperViewAction::class, $args);
            $this->redirect($url);
        }
    }
