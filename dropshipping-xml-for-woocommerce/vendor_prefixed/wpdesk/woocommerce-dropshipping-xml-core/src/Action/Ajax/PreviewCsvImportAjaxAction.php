 ...
 */
class PreviewCsvImportAjaxAction implements Conditional, Hookable
{
    ...
    public function __construct(Request $request, Renderer $renderer, FileLocatorService $file_locator, XmlAnalyser $xml_analyser, AjaxActionValidatorService $validator_service)
    {
    ...
    private function validate_form_data()
    {
        ...
        $post_data = $this->request->get_param('post.' . ImportCsvSelectorFormFields::ITEM_NUMBER);
        if (!$post_data->isSet()) {
    ...
    {
        return ['page' => (int) $this->request->get_param('post.' . ImportCsvSelectorFormFields::ITEM_NUMBER)->get(), 'uid' => $this->request->get_param('post.uid')->getAsString()];
    }
}
