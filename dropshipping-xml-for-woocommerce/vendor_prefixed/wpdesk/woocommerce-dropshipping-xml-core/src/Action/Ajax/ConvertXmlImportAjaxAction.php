 ...
 */
class ConvertXmlImportAjaxAction implements Conditional, Hookable
{
    ...
    public function __construct(Request $request, FileConverterService $converter_service, DataProviderFactory $data_provider_factory, XmlAnalyser $analyser, FileLocatorService $file_locator, AjaxActionValidatorService $validator_service)
    {
    ...
    public function ajax_import_data()
    {
        try {
            ...
            $this->converter_service->convert_from_format(DataFormat::XML);
    ...
    private function validate_form_data()
    {
        ...
        $post_data = $this->request->get_param('post.' . ImportXmlSelectorFormFields::NODE_ELEMENT);
        if (!$post_data->isSet()) {
    ...
    {
        return ['element_id' => $this->request->get_param('post.' . ImportXmlSelectorFormFields::NODE_ELEMENT)->getAsString(), 'uid' => $this->request->get_param('post.uid')->getAsString()];
    }
}
