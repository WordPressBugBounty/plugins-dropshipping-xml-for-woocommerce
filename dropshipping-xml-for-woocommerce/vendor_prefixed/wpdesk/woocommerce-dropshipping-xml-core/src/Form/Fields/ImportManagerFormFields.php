...
class ImportManagerFormFields implements FieldProvider
{
    ...
    public function get_fields()
    {
        ...
        return [(new SelectField())->set_name(self::SELECT_MANAGE)->add_class('hs-beacon-search')->set_attribute('data-beacon_search', $beacon['import'])->set_options($this->get_manage_select_options()), (new SubmitField())->set_name(self::SUBMIT_MANAGE_APPLY)->set_label(__('Apply', 'dropshipping-xml-for-woocommerce'))->add_class('button action')->set_attribute('id', self::SUBMIT_MANAGE_APPLY), (new SelectField())->set_name(self::SELECT_MANAGE_BOTTOM)->add_class('hs-beacon-search')->set_attribute('data-beacon_search', $beacon['import'])->set_options($this->get_manage_select_options()), (new SubmitField())->set_name(self::SUBMIT_MANAGE_APPLY_BOTTOM)->set_label(__('Apply', 'dropshipping-xml-for-woocommerce'))->add_class('button action')->set_attribute('id', self::SUBMIT_MANAGE_APPLY_BOTTOM), (new NoOnceField(self::NONCE_ACTION))->set_name(self::NONCE_NAME)];
    }
