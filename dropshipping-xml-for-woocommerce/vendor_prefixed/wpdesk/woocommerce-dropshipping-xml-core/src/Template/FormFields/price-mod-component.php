...
<div class="wrap-condition single-condition flex-row">
        ...
        $single_value_columns = [PriceModificatorComponent::FIELD_XPATH, PriceModificatorComponent::FIELD_VALUE_TYPE];
        if (\is_array($items)) {
...
foreach ($value as $key => $val) {
    ...
    \ob_end_clean();
    \ob_start();
    dropshipping_price_logic_generate($val, $key, $name_prefix, $field, $renderer);
    $conditions = \ob_get_contents();
    \ob_end_clean();
    dropshipping_price_groups_generate($key, $pricing, $conditions);
}
?>
	</div>
...
$conditions = \ob_get_contents();
\ob_end_clean();
dropshipping_price_groups_generate($i, $pricing, $conditions);
?>
	</div>
</script>
