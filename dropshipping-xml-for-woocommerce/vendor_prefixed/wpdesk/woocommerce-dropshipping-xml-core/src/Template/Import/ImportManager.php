...
$display_action_url = function (\DropshippingXmlFreeVendor\WPDesk\Library\DropshippingXmlCore\Entity\Import $import) use ($plugin_helper, $nonce) {
    switch ($import->get_status()) {
        ...
        case \DropshippingXmlFreeVendor\WPDesk\Library\DropshippingXmlCore\Entity\Import::STATUS_ERROR:
            ...
            $url = $plugin_helper->generate_url_by_view(ImportManagerViewAction::class, ['activate' => $import->get_id(), 'nonce' => $nonce]);
            break;
        default:
            $anchor = \__('Stop synchronization', 'dropshipping-xml-for-woocommerce');
            $url = $plugin_helper->generate_url_by_view(ImportManagerViewAction::class, ['stop' => $import->get_id(), 'nonce' => $nonce]);
            break;
    }
...
if (empty($imports)) {
    ...
    foreach ($imports as $import_file) {
        ...
        echo \esc_html(ImportManagerForm::ID);
        ?>[id][]" value="<?php 
        echo \esc_html($import_file->get_id());
        ?>">
						</th>
        ...
        ?>
							...
							<span class="edit">
									<a href="
									<?php 
        echo \esc_url($plugin_helper->generate_url_by_view(ImportFileViewAction::class, ['uid' => $import_file->get_uid(), 'mode' => 'edit_all']));
        ?>
									"><?php 
        ...
        ?></a> |
								...
								<span class="edit">
									<a href="
									<?php 
        echo \esc_url($plugin_helper->generate_url_by_view(ImportMapperViewAction::class, ['uid' => $import_file->get_uid(), 'mode' => 'edit']));
        ?>
												"><?php 
        ...
        ?></a> |
								...
								<span class="edit">
									<a href="
									<?php 
        echo \esc_url($plugin_helper->generate_url_by_view(ImportOptionsViewAction::class, ['uid' => $import_file->get_uid(), 'mode' => 'edit']));
        ?>
												"><?php 
        ...
        ?></a> |
								...
								<span class="edit">
									<a href="
									<?php 
        echo \esc_url($plugin_helper->generate_url_by_view(ImportManagerViewAction::class, ['clone' => $import_file->get_uid(), 'nonce' => $nonce]));
        ?>
												"><?php 
        ...
        ?></a> |
								...
								<span class="trash">
									<a href="
									<?php 
        echo \esc_url($plugin_helper->generate_url_by_view(ImportManagerViewAction::class, ['delete' => $import_file->get_id(), 'nonce' => $nonce]));
        ?>
												"><?php 
						...
						<td class="id column-id">
							<ul>
								<li>
									<strong>
										<a href="<?php 
        echo \esc_url($plugin_helper->generate_url_by_view(ImportStatusViewAction::class, ['uid' => $import_file->get_uid()]));
        ?>"><?php 
