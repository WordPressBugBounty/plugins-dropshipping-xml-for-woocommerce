...
class CurlHttpClient implements Connector
{
     ...
     */
    private $headers = [];
    public function __construct(CurlHttpClientOptions $options)
    {
    ...
    public function get_file(string $destination): FileObject
    {
    ...
    private function send(CurlHttpClientOptions $options)
    {
        try {
            $this->curl_init();
    ...
    private function prepare_connection(CurlHttpClientOptions $options)
    {
        curl_setopt_array($this->curl_resource, $options->get_options());
    }
    private function send_request()
    {
        $this->raw_response = trim(curl_exec($this->curl_resource));
    ...
    private function throwExceptionIfError()
    {
        ...
        if ($error_number !== 0) {
            ...
            $this->close_connection();
            throw new CurlHttpException($error_message);
        }
