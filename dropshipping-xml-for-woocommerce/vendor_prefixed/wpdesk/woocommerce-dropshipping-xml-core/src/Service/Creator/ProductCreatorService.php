...
{
    const TITLE_TYPES = [ImportOptionsFormFields::UNIQUE_PRODUCT_SELECTOR_NAME, ImportMapperFormFields::VARIATION_TYPE_TITLE_VALUE];
    const SKU_TYPES = [ImportOptionsFormFields::UNIQUE_PRODUCT_SELECTOR_SKU, ImportMapperFormFields::VARIATION_TYPE_SKU_VALUE];
    const CUSTOM_ID_TYPES = [ImportMapperFormFields::VARIATION_TYPE_CUSTOM_VALUE];
    const GROUPED_TYPES = [ImportMapperFormFields::VARIATION_TYPE_GROUP_VALUE];
    ...
    public function __construct(DataProviderFactory $data_provider_factory, MapperServiceFactory $product_mapper_factory, XmlAnalyser $analyser, ProductDAO $product_dao, ImportLoggerService $logger, FileLocatorService $file_locator, ConditionalLogicServiceFactory $conditional_logic_service_factory, ProductVariationFactory $variation_factory)
    {
    ...
    public function create_product_from_import(Import $file_import): WC_Product
    {
        $mapper = $this->create_mapper($this->data_provider_factory->create_by_class_name(ImportMapperDataProvider::class, ['postfix' => $file_import->get_uid()]), $this->data_provider_factory->create_by_class_name(ImportOptionsDataProvider::class, ['postfix' => $file_import->get_uid()]), $this->create_product_analyser_from_import($file_import));
        if (CheckboxField::VALUE_TRUE === $mapper->get_raw_option_value(ImportOptionsFormFields::FIELD_TURN_ON_LOGICAL_CONDITION)) {
            $conditional_service = $this->conditional_logic_service_factory->create_by_class_name(ConditionalLogicService::class, ['mapper' => $mapper]);
            if (!$conditional_service->is_valid()) {
                throw new ConditionalLogicException(__('The product does not meet the conditional logic requiments.', 'dropshipping-xml-for-woocommerce'));
            }
        }
        $is_variable = ImportMapperFormFields::PRODUCT_TYPE_OPTION_VARIABLE === $mapper->get_raw_value(ImportMapperFormFields::PRODUCT_TYPE);
        if ($is_variable) {
    ...
    private function is_variation(ImportMapperServiceInterface $mapper): bool
    {
    ...
    private function create_product_variable_from_import(Import $file_import, ImportMapperServiceInterface $mapper)
    {
        ...
        try {
            ...
            if (CheckboxField::VALUE_TRUE === $mapper->get_raw_option_value(ImportOptionsFormFields::FIELD_UPDATE_ONLY_EXISTING_PRODUCTS)) {
    ...
    private function has_embedded_variations(ImportMapperServiceInterface $mapper): bool
    {
        return ImportMapperFormFields::VARIATION_TYPE_EMBEDDED_VALUE === $mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE);
    }
    private function create_product_simple_from_import(Import $file_import, ImportMapperServiceInterface $mapper): WC_Product
    {
        $is_external = ImportMapperFormFields::PRODUCT_TYPE_OPTION_EXTERNAL === $mapper->get_raw_value(ImportMapperFormFields::PRODUCT_TYPE);
        try {
            $this->is_created = \false;
            $this->logger->notice(__('Searching product in the database.', 'dropshipping-xml-for-woocommerce'));
    ...
    private function update_import_information(WC_Product $wc_product, Import $file_import): WC_Product
    {
    ...
    private function update_product_variable_information(WC_Product $variable_product, Import $file_import, ImportMapperServiceInterface $mapper, ImportMapperServiceInterface $variable_mapper): WC_Product
    {
        $selector = $mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE);
        switch ($selector) {
            case ImportMapperFormFields::VARIATION_TYPE_CUSTOM_VALUE:
                $custom_id_value = $variable_mapper->map(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_XPATH);
                ...
                break;
            case ImportMapperFormFields::VARIATION_TYPE_GROUP_VALUE:
                $group_value = $variable_mapper->map(ImportMapperFormFields::VARIATION_TYPE_GROUP_XPATH);
                if (!empty($group_value)) {
                    $variable_product->update_meta_data($this->product_dao->generate_group_id($file_import->get_uid()), $group_value);
                }
                break;
        }
        $variable_product->update_meta_data(ProductDAO::HAS_PARENT_META_KEY, $this->is_variation_has_parent($mapper) ? ProductDAO::HAS_PARENT_VALUE_YES : ProductDAO::HAS_PARENT_VALUE_NO);
        $variable_product->update_meta_data(ProductDAO::RESYNC_META_KEY, ProductDAO::RESYNC_VALUE_YES);
        return $variable_product;
    }
    private function is_variation_has_parent(ImportMapperServiceInterface $mapper): bool
    {
        $selector = $mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE);
        if ($selector === ImportMapperFormFields::VARIATION_TYPE_TITLE_VALUE) {
            if (CheckboxField::VALUE_FALSE === $mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE_TITLE_PARENT_EXISTS)) {
                return \false;
            }
        } elseif ($selector === ImportMapperFormFields::VARIATION_TYPE_GROUP_VALUE) {
            if (CheckboxField::VALUE_FALSE === $mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE_GROUP_PARENT_EXISTS)) {
                return \false;
            }
        }
        return \true;
    ...
    private function find_product_variable(ImportMapperServiceInterface $mapper, Import $file_import): WC_Product
    {
        $selector = $mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE);
        $keyword = '';
        switch ($selector) {
            case ImportMapperFormFields::VARIATION_TYPE_SKU_VALUE:
                $keyword = trim($mapper->map(ImportMapperFormFields::VARIATION_TYPE_SKU_PARENT_XPATH));
                if (empty($keyword)) {
                    $keyword = $mapper->map(ImportMapperFormFields::PRODUCT_SKU);
                }
                break;
            case ImportMapperFormFields::VARIATION_TYPE_CUSTOM_VALUE:
                $keyword = $mapper->map(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_PARENT_XPATH);
                if (empty($keyword)) {
                    $keyword = $mapper->map(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_XPATH);
                }
                break;
            case ImportMapperFormFields::VARIATION_TYPE_GROUP_VALUE:
                $keyword = $mapper->map(ImportMapperFormFields::VARIATION_TYPE_GROUP_XPATH);
                break;
            default:
                $keyword = $mapper->map(ImportMapperFormFields::TITLE);
        }
        return $this->find_product_in_db($selector, wc_clean(trim($keyword)), $file_import->get_uid());
    }
    private function create_variable_mapper(ImportMapperServiceInterface $mapper, Import $file_import): ImportMapperServiceInterface
    {
        $search = '';
        $field_xpath = '';
        $new_mapper = clone $mapper;
        $selector = $new_mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE);
        try {
            switch ($selector) {
                case ImportMapperFormFields::VARIATION_TYPE_SKU_VALUE:
                    $field = $new_mapper->map(ImportMapperFormFields::PRODUCT_SKU);
                    $search = $new_mapper->map(ImportMapperFormFields::VARIATION_TYPE_SKU_PARENT_XPATH);
                    $field_xpath = $new_mapper->get_raw_value(ImportMapperFormFields::PRODUCT_SKU);
                    $parent_field_xpath = $new_mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE_SKU_PARENT_XPATH);
                    if ($field == $search || $field_xpath == $parent_field_xpath || empty($field)) {
                        return $new_mapper;
                    }
                    break;
                case ImportMapperFormFields::VARIATION_TYPE_CUSTOM_VALUE:
                    $field = $new_mapper->map(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_XPATH);
                    $search = $new_mapper->map(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_PARENT_XPATH);
                    $field_xpath = $new_mapper->get_raw_value(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_XPATH);
                    $parent_field_xpath = $new_mapper->get_raw_value(ImportMapperFormFields::VARIATION_JOIN_CUSTOM_PARENT_XPATH);
                    if ($field == $search || $field_xpath == $parent_field_xpath || empty($field)) {
                        return $new_mapper;
                    }
                    break;
                case ImportMapperFormFields::VARIATION_TYPE_GROUP_VALUE:
                    $search = $new_mapper->map(ImportMapperFormFields::VARIATION_TYPE_GROUP_XPATH);
                    $field_xpath = $new_mapper->get_raw_value(ImportMapperFormFields::VARIATION_TYPE_GROUP_XPATH);
                    break;
                default:
                    $search = $new_mapper->map(ImportMapperFormFields::TITLE);
                    $field_xpath = $new_mapper->get_raw_value(ImportMapperFormFields::TITLE);
            }
    ...
    private function create_analyser_from_xpath(string $root_element, string $xpath, string $search): XmlAnalyser
    {
    ...
    private function find_product(ImportMapperServiceInterface $mapper, Import $file_import): WC_Product
    {
        $options_data_provider = $this->data_provider_factory->create_by_class_name(ImportOptionsDataProvider::class, ['postfix' => $file_import->get_uid()]);
        $selector = $options_data_provider->get(ImportOptionsFormFields::UNIQUE_PRODUCT_SELECTOR);
        $keyword = '';
        if ($selector === ImportOptionsFormFields::UNIQUE_PRODUCT_SELECTOR_NAME) {
    ...
    private function map_product_variation(\WC_Product_Variable $wc_product, ImportMapperServiceInterface $mapper, Import $file_import): WC_Product
    {
        $parameters = ['mapper' => $mapper];
        $product_mapper = $this->mapper_factory->create_product_mapper(ProductMapperService::class, $parameters);
    ...
    private function map_product_embedded_variation(\WC_Product_Variable $wc_product, ImportMapperServiceInterface $mapper, Import $file_import): WC_Product
    {
        $variations_objects = [];
        $xpath = $mapper->get_raw_value(VariationComponent::PRODUCT_VARIATION_XPATH, ImportMapperFormFields::VARIATION_EMBEDDED);
        $xpath = trim(str_replace(['{', '}'], '', $xpath));
        ...
        if (empty($variations_objects)) {
            $create_as_simple = CheckboxField::VALUE_TRUE === $mapper->get_raw_value(VariationComponent::PRODUCT_CREATE_AS_SIMPLE, ImportMapperFormFields::VARIATION_EMBEDDED);
            ...
            foreach ($variations_objects as $variation_object) {
                ...
                $product_mapper = $this->mapper_factory->create_product_mapper(ProductEmbeddedMapperService::class, $parameters);
                $product_attr_mapper = $this->mapper_factory->create_product_mapper(ProductEmbeddedAttributeMapperService::class, $parameters);
                $product_image_mapper = $this->mapper_factory->create_product_mapper(ProductEmbeddedImageMapperService::class, $parameters);
        ...
        if (is_object($first_variation) && CheckboxField::VALUE_TRUE === $mapper->get_raw_value(VariationComponent::PRODUCT_CREATE_AS_SIMPLE, ImportMapperFormFields::VARIATION_EMBEDDED)) {
            $default_attributes = [];
    ...
    private function map_product(\WC_Product $product, ImportMapperServiceInterface $mapper): WC_Product
    {
        $parameters = ['mapper' => $mapper];
        $product_mapper = $this->mapper_factory->create_product_mapper(ProductMapperService::class, $parameters);
        $product_attr_mapper = $this->mapper_factory->create_product_mapper(ProductAttributeMapperService::class, $parameters);
    ...
    private function create_mapper(ImportMapperDataProvider $mapper_data_provider, ImportOptionsDataProvider $options_data_provider, XmlAnalyser $analyser): ImportMapperServiceInterface
    {
        $mapper = $this->mapper_factory->create_import_mapper(ImportMapperService::class, ['mapper_data_provider' => $mapper_data_provider, 'options_data_provider' => $options_data_provider]);
        $mapper->add_analyser($analyser);
        return $mapper;
    }
    private function create_product_analyser_from_import(Import $file_import): XmlAnalyser
    {
    ...
    private function create_product_analyser(string $xml_content): XmlAnalyser
    {
        $element_analyser = new XmlAnalyser();
