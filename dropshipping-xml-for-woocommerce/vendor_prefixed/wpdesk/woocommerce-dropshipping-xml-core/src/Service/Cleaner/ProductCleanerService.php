...
class ProductCleanerService
{
    ...
    const MAX_PRODUCTS_TO_CHECK = 200;
    /**
    ...
    public function __construct(DataProviderFactory $data_provider_factory, ProductDAO $product_dao, WC_Product_Factory $product_factory)
    {
    ...
    public function clean(Import $file_import): Import
    {
        $mapper_data_provider = $this->data_provider_factory->create_by_class_name(ImportMapperDataProvider::class, ['postfix' => $file_import->get_uid()]);
        $this->options_data_provider = $this->data_provider_factory->create_by_class_name(ImportOptionsDataProvider::class, ['postfix' => $file_import->get_uid()]);
    ...
    private function product_variation_cleaner(Import $file_import): array
    {
    ...
    private function product_variable_sync(Import $file_import): array
    {
        ...
        foreach ($products as $product) {
            ...
            if ($product instanceof WC_Product_Variable) {
                if ($product->get_meta(ProductDAO::HAS_PARENT_META_KEY) === ProductDAO::HAS_PARENT_VALUE_NO) {
                    $product = $this->regenerate_images_for_product_variable($product);
                }
                $product = $this->regenerate_attributes($product);
                WC_Product_Variable::sync($product);
            }
            $product->update_meta_data(ProductDAO::RESYNC_META_KEY, ProductDAO::RESYNC_VALUE_NO);
    ...
    {
        $images = [];
        $sync_fields = $this->options_data_provider->has(ImportOptionsFormFields::SYNC_FIELD) ? $this->options_data_provider->get(ImportOptionsFormFields::SYNC_FIELD) : [];
        if (in_array(ImportOptionsFormFields::SYNC_FIELD_OPTION_IMAGES, $sync_fields)) {
    ...
    private function product_cleaner(Import $file_import, string $stock_action): array
    {
        $products = [];
        switch ($stock_action) {
            case ImportOptionsFormFields::OPTION_NO_PRODUCT_TRASH:
    ...
    {
        return $this->max_execution_time <= time() - $this->start_time;
    }
    private function finish_cleaner()
    {
