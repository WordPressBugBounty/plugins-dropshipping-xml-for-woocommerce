 ...
 */
class ProductAttributeMapperService implements ProductMapperServiceInterface
{
    ...
    {
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_ATTRIBUTES)) {
    ...
    protected function create_attribute_as_taxonomy(WC_Product_Attribute $attribute, string $name, array $values, int $position): WC_Product_Attribute
    {
        ...
        foreach ($options as $option) {
            if (term_exists($option, $taxonomy->name)) {
    ...
    {
        return ImportMapperFormFields::PRODUCT_ATTRIBUTE_NAME;
    }
    protected function get_attribute_value(): string
    {
        return ImportMapperFormFields::PRODUCT_ATTRIBUTE_VALUE;
    }
    ...
    {
        return CheckboxField::VALUE_TRUE === $this->mapper->get_raw_value(ImportMapperFormFields::PRODUCT_ATTRIBUTE_AS_TAXONOMY);
    }
}
