 ...
 */
class ProductEmbeddedMapperService implements ProductMapperServiceInterface
{
    /**
    ...
    public function __construct(ImportMapperService $mapper, WC_Product $variable, ImportLoggerService $logger, PriceModificatorService $price_modificator)
    {
    ...
    {
        $wc_product->set_virtual(CheckboxField::VALUE_TRUE === $this->mapper->get_raw_value(VariationComponent::PRODUCT_VIRTUAL, ImportMapperFormFields::VARIATION_EMBEDDED));
    ...
    {
        if (CheckboxField::VALUE_TRUE === $this->mapper->get_raw_value(VariationComponent::PRODUCT_PARENT_SELECTOR, ImportMapperFormFields::VARIATION_EMBEDDED)) {
            $options = $this->mapper->get_raw_value(VariationComponent::PRODUCT_PARENT_OPTIONS, ImportMapperFormFields::VARIATION_EMBEDDED);
    ...
    {
        $description = '';
        if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_DESCRIPTION, ImportMapperFormFields::VARIATION_EMBEDDED)) {
            $description = $this->mapper->map(VariationComponent::PRODUCT_DESCRIPTION, ImportMapperFormFields::VARIATION_EMBEDDED);
        }
        if (\is_string($description)) {
            $wc_product->set_description(apply_filters(ProductMapperService::FILTER_NAME_DESCRIPTION, $description));
        }
        return $wc_product;
    }
    private function update_sku(WC_Product $wc_product): WC_Product
    {
        ...
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_STOCK_SKU)) {
            if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_SKU, ImportMapperFormFields::VARIATION_EMBEDDED)) {
                $sku = $this->mapper->map(VariationComponent::PRODUCT_SKU, ImportMapperFormFields::VARIATION_EMBEDDED);
    ...
    private function update_tax(WC_Product $wc_product): WC_Product
    {
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_GENERAL_TAX_STATUS)) {
            $tax_status = $this->mapper->map(ImportMapperFormFields::PRODUCT_TAX_STATUS, ImportMapperFormFields::VARIATION_EMBEDDED);
            if (!empty($tax_status)) {
    ...
    private function update_shipping(WC_Product $wc_product): WC_Product
    {
        ...
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_SHIPPING_WEIGHT)) {
            if ($this->is_field_mapped_by_parent(VariationComponent::PRODUCT_PARENT_OPTIONS_WEIGHT_VALUE)) {
                $wc_product->set_weight($this->variable->get_weight());
            } else {
                if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_WEIGHT, ImportMapperFormFields::VARIATION_EMBEDDED)) {
                    $weight = apply_filters(ProductMapperService::FILTER_NAME_WEIGHT, $this->mapper->map(VariationComponent::PRODUCT_WEIGHT, ImportMapperFormFields::VARIATION_EMBEDDED));
        ...
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_SHIPPING_DIMENSIONS)) {
            if ($this->is_field_mapped_by_parent(VariationComponent::PRODUCT_PARENT_OPTIONS_DIMENSIONS_VALUE)) {
                ...
                if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_LENGTH, ImportMapperFormFields::VARIATION_EMBEDDED)) {
                    $length = $this->mapper->map(VariationComponent::PRODUCT_LENGTH, ImportMapperFormFields::VARIATION_EMBEDDED);
                    if ($length !== '') {
                ...
                if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_WIDTH, ImportMapperFormFields::VARIATION_EMBEDDED)) {
                    $width = $this->mapper->map(VariationComponent::PRODUCT_WIDTH, ImportMapperFormFields::VARIATION_EMBEDDED);
                    if ($width !== '') {
                ...
                if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_HEIGHT, ImportMapperFormFields::VARIATION_EMBEDDED)) {
                    $height = $this->mapper->map(VariationComponent::PRODUCT_HEIGHT, ImportMapperFormFields::VARIATION_EMBEDDED);
                    if ($height !== '') {
        ...
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_SHIPPING_CLASS)) {
            if ($this->is_field_mapped_by_parent(VariationComponent::PRODUCT_PARENT_OPTIONS_SHIPPING_VALUE)) {
                $wc_product->set_shipping_class_id($this->variable->get_shipping_class_id());
            } else {
                if ($this->mapper->has_value_to_map(VariationComponent::PRODUCT_SHIPPING_CLASS, ImportMapperFormFields::VARIATION_EMBEDDED)) {
                    $shipping_class_id = $this->mapper->map(VariationComponent::PRODUCT_SHIPPING_CLASS, ImportMapperFormFields::VARIATION_EMBEDDED);
                }
    ...
    private function update_stock(WC_Product $wc_product): WC_Product
    {
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_STOCK_MANAGMENT)) {
            if ($this->is_field_mapped_by_parent(VariationComponent::PRODUCT_PARENT_OPTIONS_STOCK_VALUE)) {
                ...
                $stock_enabled = CheckboxField::VALUE_TRUE === $this->mapper->map(VariationComponent::PRODUCT_MANAGE_STOCK, ImportMapperFormFields::VARIATION_EMBEDDED);
                $wc_product->set_manage_stock($stock_enabled);
                if ($stock_enabled) {
                    $has_stock_value = $this->mapper->has_value_to_map(VariationComponent::PRODUCT_STOCK, ImportMapperFormFields::VARIATION_EMBEDDED);
                    $stock = $this->mapper->map(VariationComponent::PRODUCT_STOCK, ImportMapperFormFields::VARIATION_EMBEDDED);
                    if ($stock !== '') {
                        $stock = $this->format_number(apply_filters(ProductMapperService::FILTER_NAME_STOCK, $stock));
                    }
    ...
    {
        $raw_price = $this->mapper->map(VariationComponent::PRODUCT_PRICE, ImportMapperFormFields::VARIATION_EMBEDDED);
        $result = 0;
        $price = $this->format_number($raw_price);
        $price = apply_filters(ProductMapperService::FILTER_NAME_PRICE_BEFORE_MOD, $price, $raw_price);
        $price = (float) $price;
        $result = $this->price_modificator->get_regular_price($price);
        $result = apply_filters(ProductMapperService::FILTER_NAME_PRICE_AFTER_MOD, $result);
        return (float) $result;
    }
    private function get_sale_price(): float
    {
        $raw_price = $this->mapper->map(ImportMapperFormFields::PRODUCT_SALE_PRICE, ImportMapperFormFields::VARIATION_EMBEDDED);
        $result = 0;
        $price = $this->format_number($raw_price);
        $price = apply_filters(ProductMapperService::FILTER_NAME_PRICE_BEFORE_MOD, $price, $raw_price);
        $price = (float) $price;
    ...
    {
        $result = '';
        $tax_classes = $this->mapper->get_raw_value(VariationComponent::PRODUCT_TAX_CLASS_MULTI_MAP, ImportMapperFormFields::VARIATION_EMBEDDED);
        $items_nr = is_array(reset($tax_classes)) ? count(reset($tax_classes)) : 1;
        for ($i = 0; $i < $items_nr; $i++) {
            if (isset($tax_classes[VariationComponent::PRODUCT_TAX_CLASS_MULTI_MAP_ID][$i]) && isset($tax_classes[VariationComponent::PRODUCT_TAX_CLASS_MULTI_MAP_VALUE][$i])) {
                $tax_class_id = trim($tax_classes[VariationComponent::PRODUCT_TAX_CLASS_MULTI_MAP_ID][$i]);
                $value = trim($tax_classes[VariationComponent::PRODUCT_TAX_CLASS_MULTI_MAP_VALUE][$i]);
    ...
    private function get_tax_class(): string
    {
        ...
        $single = CheckboxField::VALUE_FALSE === $this->mapper->get_raw_value(VariationComponent::PRODUCT_TAX_CLASS_XPATH_SWITCHER, ImportMapperFormFields::VARIATION_EMBEDDED);
        if ($single) {
            $result = $this->mapper->get_raw_value(VariationComponent::PRODUCT_TAX_CLASS, ImportMapperFormFields::VARIATION_EMBEDDED);
        } else {
            $mapped_tax_class_field = $this->mapper->get_raw_value(VariationComponent::PRODUCT_TAX_CLASS_MAPPER_FIELD, ImportMapperFormFields::VARIATION_EMBEDDED);
