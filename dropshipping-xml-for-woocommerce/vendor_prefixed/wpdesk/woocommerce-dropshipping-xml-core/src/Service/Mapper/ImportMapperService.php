 ...
 */
class ImportMapperService implements ImportMapperServiceInterface
{
    /**
    ...
    public function __construct(ImportMapperDataProvider $mapper_data_provider, ImportOptionsDataProvider $options_data_provider, array $analysers = [])
    {
    ...
    public function set_analysers(array $analysers)
    {
        ...
        if (!empty($this->analysers)) {
            foreach ($analysers as $analyser) {
                if (\is_object($analyser) && $analyser instanceof XmlAnalyser) {
    ...
    public function add_analyser(XmlAnalyser $analyser, bool $add_as_first = \false)
    {
        if (\true === $add_as_first) {
    ...
    public function map(string $key, string $group = ''): string
    {
        ...
        if ($this->has_value_to_map($key, $group)) {
            return $this->get_mapped_content(trim($this->get_raw_value($key, $group)));
        }
        return '';
    }
    ...
    private function get_synced_product_fields(): array
    {
        ...
        if ($this->options_data_provider->has(ImportOptionsFormFields::SYNC_FIELD)) {
            $params = $this->options_data_provider->get(ImportOptionsFormFields::SYNC_FIELD);
        } else {
            $params = array_keys(ImportOptionsFormFields::get_grouped_fields());
            if (CheckboxField::VALUE_TRUE === $this->get_raw_value(ImportMapperFormFields::PRODUCT_SHIPPING_CLASS_SYNC_DISABLED)) {
                if (isset($params[ImportOptionsFormFields::SYNC_FIELD_OPTION_SHIPPING_CLASS])) {
                    unset($params[ImportOptionsFormFields::SYNC_FIELD_OPTION_SHIPPING_CLASS]);
                }
            }
            if (CheckboxField::VALUE_TRUE === $this->get_raw_value(ImportMapperFormFields::PRODUCT_CATEGORIES_SYNC_DISABLED)) {
                if (isset($params[ImportOptionsFormFields::SYNC_FIELD_OPTION_CATEGORIES])) {
                    unset($params[ImportOptionsFormFields::SYNC_FIELD_OPTION_CATEGORIES]);
                }
            }
            if (CheckboxField::VALUE_TRUE === $this->get_raw_value(ImportMapperFormFields::PRODUCT_ATTRIBUTE_SYNC_DISABLED)) {
                if (isset($params[ImportOptionsFormFields::SYNC_FIELD_OPTION_ATTRIBUTES])) {
                    unset($params[ImportOptionsFormFields::SYNC_FIELD_OPTION_ATTRIBUTES]);
                }
            }
        }
