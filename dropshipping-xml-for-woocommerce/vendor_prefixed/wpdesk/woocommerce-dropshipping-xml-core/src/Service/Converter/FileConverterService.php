...
class FileConverterService
{
    ...
    public function __construct(DataFormat $data_format, FileConverterFactory $factory, ConverterValidatorService $validator_service)
    {
    ...
    public function is_file_converted(FileObject $source_file)
    {
        $converted_file_location = $this->get_converted_file_location($source_file);
        return file_exists($converted_file_location);
    }
    public function get_converted_file_location(FileObject $source_file)
    {
        return trailingslashit($source_file->getDirLocation()) . $source_file->getName() . '.' . DataFormat::XML;
    }
}
