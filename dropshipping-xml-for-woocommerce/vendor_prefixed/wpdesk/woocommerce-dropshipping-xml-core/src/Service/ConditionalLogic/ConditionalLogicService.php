...
class ConditionalLogicService implements ConditionalLogicServiceInterface
{
    ...
    public function __construct(ImportMapperService $mapper, ConditionalLogicFactory $conditional_logic_factory)
    {
        $this->mapper = $mapper;
    ...
    {
        $conditions = $this->mapper->get_raw_option_value(ImportOptionsFormFields::LOGICAL_CONDITIONS);
        $conditions = \is_array($conditions) ? $conditions : [];
        $count_items = is_array(reset($conditions)) ? count(reset($conditions)) : 1;
        try {
            for ($i = 0; $i < $count_items; $i++) {
                $result = \true;
                if (isset($conditions[ConditionalLogicComponent::FIELD_XPATH][$i])) {
                    $should_import = ConditionalLogicComponent::FIELD_IMPORT_OPTION_ALLOW === $conditions[ConditionalLogicComponent::FIELD_IMPORT][$i];
