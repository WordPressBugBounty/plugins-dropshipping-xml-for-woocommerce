...
namespace DropshippingXmlFreeVendor\WPDesk\License\Page\License\Action;

use DropshippingXmlFreeVendor\WPDesk\License\Page\Action;
use DropshippingXmlFreeVendor\WPDesk\License\ServerAddressRepository;
 ...
 */
class LicenseActivation extends AbstractAction implements Action
{
    /**
     * Plugin data.
     *
    ...
    {
        $activation_email = sanitize_email(trim($_POST['activation_email']));
        $api_key = sanitize_text_field(trim($_POST['api_key']));
        $product_id = $plugin['product_id'];
        $this->plugin_data = $plugin;
        $this->activate_license($activation_email, $api_key, new ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        $this->add_error('api_key_check_text', 'api_key_check_error', __('Connection failed to the Subscription Key API server. Try again later.', 'dropshipping-xml-for-woocommerce'), 'error');
    }
    /**
    ...
    {
        $this->add_error('activate_text', 'activate_msg', __('Plugin license activated. ', 'dropshipping-xml-for-woocommerce') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, ServerAddressRepository $address_repository)
    {
