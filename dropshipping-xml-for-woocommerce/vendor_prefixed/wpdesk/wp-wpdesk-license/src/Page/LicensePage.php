...
use DropshippingXmlFreeVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements Hookable
{
    ...
    public function __construct(InstalledPlugins $plugin_database)
    {
    ...
    {
        add_submenu_page('wpdesk-helper', __('Subscriptions', 'dropshipping-xml-for-woocommerce'), __('Subscriptions', 'dropshipping-xml-for-woocommerce'), 'manage_options', self::PAGE_SLUG, [$this, 'handle_render_wpdesk_licenses_page']);
    }
    /**
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!class_exists('DropshippingXmlFreeVendor\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
