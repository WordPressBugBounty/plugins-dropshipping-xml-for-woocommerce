...
class PluginRegistrator implements PluginRegistratorInterface
{
    ...
    {
        (new PluginUpgrade($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new PluginExternalBlocking($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new PluginViewVersionInfo($this->plugin_info, $this->server))->hooks();
    }
}
