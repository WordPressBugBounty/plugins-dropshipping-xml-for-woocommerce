...
namespace DropshippingXmlFreeVendor\WPDesk\License\ActivationForm;

use DropshippingXmlFreeVendor\WPDesk\License\LicenseManager;
use DropshippingXmlFreeVendor\WPDesk\License\Page\LicensePageActions;
use DropshippingXmlFreeVendor\WPDesk\License\PluginLicense;
use DropshippingXmlFreeVendor\WPDesk\PluginBuilder\Plugin\Hookable;
...
class AjaxHandler implements Hookable
{
    ...
    public function handle_ajax_action()
    {
        ...
        $license_page_action = (new LicensePageActions())->create_action($license_action, \false);
        ...
        $plugin_license = new PluginLicense($this->plugin_info);
        $form_content_renderer = new FormContentRenderer($this->plugin_info, $this->is_update_possible(isset($update_plugins[$this->plugin_info->get_plugin_file_name()]) ?? new \stdClass()), $license_page_action->get_errors());
    ...
    {
        if (isset($plugin_data->Version, $plugin_data->new_version) && $plugin_data->Version !== $plugin_data->new_version && empty($plugin_data->package)) {
            return \false;
        }
        return \true;
    }
