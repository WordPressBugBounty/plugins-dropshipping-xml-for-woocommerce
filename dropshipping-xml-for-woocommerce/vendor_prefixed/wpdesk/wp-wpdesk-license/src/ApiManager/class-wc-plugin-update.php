 ...
 *
 */
if (!\class_exists('DropshippingXmlFreeVendor\WPDesk_Update_API_Check')) {
    class WPDesk_Update_API_Check
    {
        ...
        public function plugin_information($args)
        {
            ...
            if (\is_wp_error($request) || \wp_remote_retrieve_response_code($request) != 200) {
                if (\class_exists('DropshippingXmlFreeVendor\WPDesk_Logger_Factory')) {
                    if (\is_wp_error($request)) {
                        WPDesk_Logger_Factory::log_wp_error($request, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                    } else {
                        WPDesk_Logger_Factory::log_message_backtrace('Response is invalid. Response: ' . \json_encode($request), WPDesk_Logger::ERROR, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                    }
                }
                return \false;
            }
            ...
            if (\is_object($response)) {
                ...
                if (\class_exists('DropshippingXmlFreeVendor\WPDesk_Logger_Factory')) {
                    WPDesk_Logger_Factory::log_message_backtrace('Response is not an object', WPDesk_Logger::DEBUG, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                } else {
                    \error_log(\sprintf("Unserialize error. Please send this report to support@wpdesk.net. Request: %s. Raw Response: %s", \json_encode($request), $raw_response));
                }
                return \false;
            }
        }
